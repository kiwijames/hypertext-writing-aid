...
db.all("SELECT * FROM internallinks", function(err,rows){
    if(err) {
    console.log(err)
    } else{
        console.log(rows)
            ...
            tabledata.push(
                {
                    id:row.link_id,
                    name:row.link_name,
                    document_name_1:row.doc_name,
                    document_data_2:row.doc_data,
                    document_name_2:row.pdf_name,
                    document_data_1:row.doc_data,
...
//document.addEventListener('DOMContentLoaded', function() {
    ...
    if(toReturnLinkId){
        ...
        table.addColumn({formatter:"buttonCross", width:40, align:"center", cellClick:function(e, cell){
            let row = cell.getRow()
            console.log(row)
            ipcRenderer.send('deleteInternalLink',row.getData().id);
            cell.getRow().delete();
        }});
    }
    table.redraw()
